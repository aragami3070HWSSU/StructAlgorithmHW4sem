...
- После этого идет балансировка --- $O(1)$ для каждого нарушенного узла. Но таких узлов может быть $O(log N)$ (по пути от новго узла до корня).
...
- Балансировка через повороты займет аналогичное вставке $O(log N)$ операций (по пути от удаленного узла до корня)
