...
== Анализ сложности
В худшем случае рекурсивный вызов идет по высоте кучи -- O(log n).

Построение кучи:
Несмотря на вызов heap_help для приблизительно n/2 элементов, общая сложность построения кучи -- O(n).(Результат основыввается на сумме высот всех узлов)

Сортировка (извлечение элементов):
Цикл выполняется n-1 раз, каждый раз вызывается heap_help с временной сложностью O(log n).
Итого: O(n $dot$  log n).
