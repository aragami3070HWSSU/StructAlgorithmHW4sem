...
Таким образом общая временная сложность: $O(n) + O(k) + O(k) dot (O(n) dot O(k) + O(n)) = O(n + k + k(n k + n)) = O(n + k + n k^2 + n k) = O(n k^2)$. При малом числе разрядов сложность близка к линейной.
