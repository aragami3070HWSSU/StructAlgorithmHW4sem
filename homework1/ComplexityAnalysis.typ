...
+ Цикл по всем разрядам $O(k)$. Вложеные циклы:
	- Цикл распределения элементов в вспомогательный массив --- $O(n dot k)$
	- Двойной цикл на перезаписывание вектора от $#[temp] [0]$ до $#[temp] [p - 1]$ в массив array --- $O(p dot n) = O(n)$

Таким образом общая временная сложность: $O(n) + O(k) + O(k) dot (O(n dot k) + O(n)) = O(n + k + k(n k + n)) = O(n + k + n k^2 + n k) = O(n k^2)$. При малом числе разрядов сложность близка к линейной.
